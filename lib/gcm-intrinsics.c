...
extern void gfmul(__m128i a, __m128i b,
                  __m128i *c);  // must be implemented elsewhere
...
void AES_GCM_encrypt(const unsigned char *in, unsigned char *out,
  ...
  if (ibytes == 96 / 8) {
    ...
    tmp1 = _mm_xor_si128(X, KEY[0]);
    for (j = 1; j < nr; j++) tmp1 = _mm_aesenc_si128(tmp1, KEY[j]);
    H = _mm_aesenclast_si128(tmp1, KEY[nr]);
    H = _mm_shuffle_epi8(H, BSWAP_MASK);
    Y = _mm_xor_si128(Y, Y);
  ...
  if (nbytes % 16) {
    ...
    for (j; j < 16; j++) ((unsigned char *)&last_block)[j] = 0;
    tmp1 = last_block;
    tmp1 = _mm_shuffle_epi8(tmp1, BSWAP_MASK);
    X = _mm_xor_si128(X, tmp1);
    gfmul(X, H, &X);
